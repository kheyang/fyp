...
class Trainer:
    def __init__(self, options):
        ...
        # If not using stereo images as input, use pose net
        ...
        if self.use_pose_net:
            if self.opt.pose_model_type == "separate_resnet":
                ...
                self.models["pose"] = networks.PoseDecoder(
                    self.models["depth_encoder"].num_ch_enc, self.num_pose_frames)
        ...
        self.model_lr_scheduler_surface_normal = optim.lr_scheduler.StepLR(
            self.model_optimizer_surface_normal, self.opt.scheduler_step_size, 0.1)

        if self.opt.load_weights_folder is not None:
            self.load_model()

        # Logging training information at the start of the training
        ...
        # DataLoader represents a Python iterable over the dataset
        ...
        # Loading dataset for evaluating
        ...
        for scale in self.opt.scales:
            ...
            # Back project depth transforms depth image into point clouds
            self.backproject_depth[scale] = BackprojectDepth(self.opt.batch_size, h, w)
            self.backproject_depth[scale].to(self.device)
            # Projects 3D points into a camera with intrinsics K and at position T
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in enumerate(self.train_loader):
            ...
            self.model_optimizer_depth.zero_grad()
            self.model_optimizer_surface_normal.zero_grad()
            # loss.backward() computes dloss/dx for every parameter x which has requires_grad=True
            # These are accumulated into x.grad for every parameter x
            losses["loss"].backward()
            # performs a parameter update based on the current gradient
            self.model_optimizer_depth.step()
            self.model_optimizer_surface_normal.step()
    ...
    def process_batch(self, inputs):
        ...
        if self.opt.pose_model_type == "shared":
            ...
            all_depth_features = self.models["depth_encoder"](all_color_aug)
            all_depth_features = [torch.split(f, self.opt.batch_size) for f in all_depth_features]

            features_depth = {}
            for i, k in enumerate(self.opt.frame_ids):
                features_depth[k] = [f[i] for f in all_depth_features]

            outputs = self.models["depth"](features_depth[0])
        else:
            # Otherwise, we only feed the image with frame_id 0 through the depth encoder
            features_depth = self.models["depth_encoder"](inputs["color_aug", 0, 0])
            outputs = self.models["depth"](features_depth)

        if self.opt.predictive_mask:
            outputs["predictive_mask"] = self.models["predictive_mask"](features_depth)
    ...
    def save_model(self):
        ...
        for model_name, model in self.models.items():
            ...
            if model_name == 'depth_encoder' or model_name == 'surface_normal_encoder':
        ...
        torch.save(self.model_optimizer_depth.state_dict(), save_path)
    ...
    def load_model(self):
        ...
        if os.path.isfile(optimizer_load_path):
            ...
            self.model_optimizer_depth.load_state_dict(optimizer_dict)
        else:
